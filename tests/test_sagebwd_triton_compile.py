...
from sageattention import sageattn_qk_int8_pv_fp16_triton_trainable


def _eager_autotuned_backward(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    dout: torch.Tensor,
    ...
    out = sageattn_qk_int8_pv_fp16_triton_trainable(
        q,
        k,
        v,
        tensor_layout="NHD",
...
def _compile_autotuned_backward(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    dout: torch.Tensor,
...
def test_eager_autotuned() -> None:
    ...
    actual = _eager_autotuned_backward(q, k, v, dout)
...
def test_compile_autotuned() -> None:
    q, k, v, dout = _make_qkvo()
    expected = _flash_attn_backward(q, k, v, dout)
    actual = _compile_autotuned_backward(q, k, v, dout)
