...
def _run_case(*, head_dim, dtype, tensor_layout, is_causal, smooth_k):
...
def main():
    ...
    failures = []
    tested = 0
    for head_dim, dtype, tensor_layout, is_causal, smooth_k in modes:
        tested += 1
        name = f"head_dim={head_dim} dtype={dtype} layout={tensor_layout} is_causal={is_causal} smooth_k={smooth_k}"
        try:
