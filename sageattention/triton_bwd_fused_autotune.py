...
from .triton.attn_bwd_autotune import _valid_bwd_fused_configs
from .triton_autotune import _valid_configs as _valid_forward_configs

_TRITON_TRAINABLE_FUSED_AUTOTUNE_CACHE: dict[object, tuple[int, int]] = {}
_TRITON_TRAINABLE_FUSED_COMPILE_AUTOTUNE_NAME = "_sageattention_triton_trainable_fused_autotuned"
_SELECTED_COMPILE_FUSED_BLOCK_CONFIGS: dict[object, tuple[int, int]] = {}
...
def _compile_block_config_key(
    ...
    return autotune_utils._tensor_autotune_cache_key(
        ...
        pv_accum_dtype,
        smooth_k,
        os.environ.get("SAGEATTN_FUSED_DQ_SPLITS", ""),
        "compile_fused",
    )
...
def _config_is_valid(
    ...
    return bool(_valid_bwd_fused_configs(block_config, head_dim, device_index))
...
def _normalize_config(
    ...
    if min(block_config) <= 0 or block_config not in block_configs:
        if selected_key is not None:
            selected_config = _SELECTED_COMPILE_FUSED_BLOCK_CONFIGS.get(selected_key)
...
def _eager_autotune_select(
    ...
    from .triton_bwd_fused import _sageattn_triton_trainable_fused_configured
    ...
    return autotune_utils._eager_autotune_select(
        block_configs,
        _TRITON_TRAINABLE_FUSED_AUTOTUNE_CACHE,
        key,
        benchmark,
    )


def _sageattn_triton_trainable_fused_forward_configured(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    tensor_layout: str,
...
@torch.library.custom_op("sageattention_internal::sageattn_triton_trainable_fused_autotuned", mutates_args=())
def _sageattn_triton_trainable_fused_autotuned(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    tensor_layout: str,
    ...
    return _sageattn_triton_trainable_fused_forward_configured(
        q,
        k,
        v,
        tensor_layout,
...
@_sageattn_triton_trainable_fused_autotuned.register_fake
def _(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
...
@torch.library.custom_op("sageattention_internal::sageattn_triton_trainable_fused_backward", mutates_args=())
def _sageattn_triton_trainable_fused_backward(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    dout: torch.Tensor,
    ...
    from .triton_bwd_fused import _trainable_fused_backward_from_inputs

    selected_key = _compile_block_config_key(q, k, v, tensor_layout, pv_accum_dtype, smooth_k)
    block_config = _normalize_config(q, block_m, block_n, selected_key)
    return _trainable_fused_backward_from_inputs(
        q,
        k,
        v,
        dout,
...
@_sageattn_triton_trainable_fused_backward.register_fake
def _(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
...
def _sageattn_triton_trainable_fused_setup_context(ctx, inputs, output) -> None:
...
def _sageattn_triton_trainable_fused_autograd(ctx, dout: torch.Tensor):
    q, k, v = ctx.saved_tensors
    dq, dk, dv = _sageattn_triton_trainable_fused_backward(
        q,
        k,
        v,
        dout,
...
torch.library.register_autograd(
    _sageattn_triton_trainable_fused_autotuned,
    _sageattn_triton_trainable_fused_autograd,
    setup_context=_sageattn_triton_trainable_fused_setup_context,
)


def _record_compile_trainable_fused_selection(choice) -> None:
    ...
    _SELECTED_COMPILE_FUSED_BLOCK_CONFIGS[key] = (kwargs["block_m"], kwargs["block_n"])


def _compile_trainable_fused_timing_target(choice, inputs: tuple[torch.Tensor, ...], out: torch.Tensor) -> float:
    kwargs = choice.decomposition_kwargs
    ...
    def fn() -> None:
        ...
        with torch.enable_grad():
            output = _sageattn_triton_trainable_fused_autotuned(q, k, v, **kwargs)
...
register_custom_timing_target(
    _TRITON_TRAINABLE_FUSED_COMPILE_AUTOTUNE_NAME,
    _compile_trainable_fused_timing_target,
    on_select=_record_compile_trainable_fused_selection,
)
...
register_custom_op_autotuning(
    ...
    name=_TRITON_TRAINABLE_FUSED_COMPILE_AUTOTUNE_NAME,
)
