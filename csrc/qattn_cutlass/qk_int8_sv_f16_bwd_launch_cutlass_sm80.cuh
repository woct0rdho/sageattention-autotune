                                    ...
                                    const int64_t warp_k)
{
  ...
  const int32_t q_scale_blocks = div_ceil_int(params.seq_len, static_cast<int32_t>(blk_q)) * static_cast<int32_t>(blk_q / warp_q) * 8;
  const int32_t k_scale_blocks = div_ceil_int(params.seq_len, static_cast<int32_t>(blk_k)) * static_cast<int32_t>(blk_k / warp_k) * 4;
...
template <int32_t HeadDim, int32_t BlockM, int32_t BlockN, int32_t NumWarps>
...
{
  static_assert(BlockM > 0 && BlockN > 0 && NumWarps > 0, "Backward block config must be positive");
  using Traits = BwdTileTraits<HeadDim, BlockM, BlockN, NumWarps>;
  using MicroTraits = BwdTileTraits<HeadDim>;
  ...
  const int32_t kv_blocks = div_ceil_int(params.seq_len, Traits::kCtaN);

  constexpr int32_t dim_blocks = HeadDim / MicroTraits::kBlockK;
  ...
  if constexpr (HeadDim == 64 && BlockM == 64 && BlockN == 64 && NumWarps == 4)
  {
    constexpr int32_t kKernelWarps = 8;
    using KernelTraits = BwdTileTraits<HeadDim, BlockM, BlockN, kKernelWarps>;
    auto kernel = fused_mma_kernel_2d_warp<HeadDim, BlockM, BlockN, kKernelWarps>;
  ...
  {
    auto kernel = fused_mma_kernel<HeadDim, BlockM, BlockN, NumWarps>;
    constexpr int32_t smem_size = sizeof(SharedStorage<Traits>);
    if constexpr (smem_size >= 48 * 1024)
    {
      cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    }
    kernel<<<kv_grid, dim3(32, NumWarps), smem_size, stream>>>(
      reinterpret_cast<const int8_t*>(query.const_data_ptr()),
      reinterpret_cast<const int8_t*>(key.const_data_ptr()),
      reinterpret_cast<const float*>(query_scale.const_data_ptr()),
      reinterpret_cast<const float*>(key_scale.const_data_ptr()),
                                               ...
                                               const int64_t warp_k)
{
  ...
  STD_TORCH_CHECK(params.head_dim == 64 || params.head_dim == 128,
                  "CUTLASS qattn backward currently supports head_dim 64 or 128");
