...
extern template void launch_mma<128, 64, 64, 8, 32, 64>(
  const torch::stable::Tensor &query,
  const torch::stable::Tensor &key,
  const torch::stable::Tensor &query_scale,
  const torch::stable::Tensor &key_scale,
                                  ...
                                  const bool smooth_k)
{
  ...
  if (params.head_dim == 128)
  {
      ...
      {
        launch_mma<128, 64, 64, 8, 32, 64>(query, key, query_scale, key_scale, value, dO, lse, delta, dQ_accum, dS_sum, dO_int8, dO_scale, dS_q_factors, dS_k_factors, dK, dV, params, sm_scale, smooth_k);
        return;
      }
  }
