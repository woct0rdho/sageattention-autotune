...
// sm89 (Ada / RTX 40xx) and sm120 (Blackwell / RTX 50xx) int8-QK / fp8-PV
// attention kernel. Adapted from upstream SageAttention's
// qk_int_sv_f8_cuda_sm89.cuh into this fork's runtime-block-size /
// stable-ABI conventions: the DataType and QuantGranularity template
// parameters are dropped (int8 QK with per-thread scales only), matching
// the fork's sm80 kernel and Triton per-thread quantizer.
...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim,
         ...
         ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false,
         bool fuse_v_scale = false, bool fuse_v_mean = false, bool use_pv_fp16_accu = false>
__global__ void qk_int8_sv_f8_attn_kernel(const int8_t *__restrict__ Q, const int8_t *__restrict__ K, const int8_t *__restrict__ V, DTypeOut *__restrict__ O, float *__restrict__ Lse,
                      const float *__restrict__ Q_scale, const float *__restrict__ K_scale, const float *__restrict__ V_scale, const float *__restrict__ V_mean,
...
{
  // compile time check
  static_assert(std::is_same<DTypeSVAccum, float>::value, "DTypeSVAccum must be float (half accum is WIP)");
  ...
  // per-thread quantization scale indices (matches the fork's sm80 kernel and per_thread_int8)
...
#pragma unroll
  ...
  // V is stored transposed (head_dim rows, kv_len columns); load along stride_d_v
  const int8_t *V_lane_base_ptr = V + batch_id * stride_bz_v + (head_id / num_kv_groups) * stride_h_v + head_dim / num_warps * warp_id * stride_d_v + lane_id / global_to_shared_line_lanes_V * stride_d_v + (lane_id % global_to_shared_line_lanes_V) * PACK_SIZE_V;
...
#pragma unroll
  ...
  // load V (assumes V is padded along kv_len to a multiple of CTA_K)
...
#pragma unroll
  ...
  // dequantize V (per-channel fp8 scale), fused into the output
  if constexpr (fuse_v_scale)
  {
    float v_scale[4];
    const float *V_scale_base_ptr = V_scale + batch_id * (num_qo_heads / num_kv_groups) * head_dim + (head_id / num_kv_groups) * head_dim + (lane_id % 4) * 2;
#pragma unroll
    for (uint32_t fv = 0; fv < num_tiles_v; fv++)
    {
...
#pragma unroll
  ...
  {
    float v_mean[4];
    const float *V_mean_base_ptr = V_mean + batch_id * (num_qo_heads / num_kv_groups) * head_dim + (head_id / num_kv_groups) * head_dim + (lane_id % 4) * 2;
#pragma unroll
    for (uint32_t fv = 0; fv < num_tiles_v; fv++)
    {
