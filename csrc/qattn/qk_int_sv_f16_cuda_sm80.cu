...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
...
__global__ void qk_int_sv_f16_attn_kernel(const int8_t *__restrict__ Q, const int8_t *__restrict__ K, const half *__restrict__ V, DTypeOut *__restrict__ O, float *__restrict__ Lse,
                      const float *__restrict__ Q_scale, const float *__restrict__ K_scale, const DTypeOut *__restrict__ V_mean,
...
{
  // compile time check
  static_assert(Q_GRAN == QuantGranularity::kPerWarp || Q_GRAN == QuantGranularity::kPerThread, "Q_GRAN must be kPerWarp or kPerThread");
  static_assert(K_GRAN == QuantGranularity::kPerWarp || K_GRAN == QuantGranularity::kPerThread, "K_GRAN must be kPerWarp or kPerThread");
  ...
  constexpr uint32_t k_scale_advance_offset = (K_GRAN == QuantGranularity::kPerWarp) ? (CTA_K / WARP_K) : (CTA_K / WARP_K) * 4;
...
#pragma unroll
  ...
  const int8_t *Q_lane_base_ptr = Q + batch_id * stride_bz_q + head_id * stride_h_q + (bx * CTA_Q + CTA_Q / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK) * stride_seq_q + (lane_id % global_to_shared_line_lanes_QK) * PACK_SIZE_QK;
  const int8_t *K_lane_base_ptr = K + batch_id * stride_bz_k + (head_id / num_kv_groups) * stride_h_k + (CTA_K / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK) * stride_seq_k + (lane_id % global_to_shared_line_lanes_QK) * PACK_SIZE_QK;
  const half *V_lane_base_ptr = V + batch_id * stride_bz_v + (head_id / num_kv_groups) * stride_h_v + (CTA_K / num_warps * warp_id + lane_id / global_to_shared_line_lanes_V) * stride_seq_v + (lane_id % global_to_shared_line_lanes_V) * PACK_SIZE_V;
...
#pragma unroll
    ...
    if constexpr (std::is_same<DTypeSVAccum, float>::value)
    {
      update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, false>(RS_f32, RO, m, d, sm_scale);
    }
    else if constexpr (std::is_same<DTypeSVAccum, half>::value)
    {
      update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, true>(RS_f32, RO, m, d, sm_scale);
    }
...
#pragma unroll
    ...
    {
      update_mdo<num_tiles_q, num_tiles_k, num_tiles_v, true>(RS_f32, RO, m, d, original_sm_scale);
    }
...
#pragma unroll
  ...
  {
    DTypeOut2 v_mean[2];
    const DTypeOut *V_mean_lane_ptr = V_mean + batch_id * (num_qo_heads / num_kv_groups) * head_dim + (head_id / num_kv_groups) * head_dim + lane_id % 4 * 2;
#pragma unroll
    for (uint32_t fv = 0; fv < num_tiles_v; fv++)
    {
      v_mean[0] = *reinterpret_cast<const DTypeOut2*>(V_mean_lane_ptr + fv * 16);
      v_mean[1] = *reinterpret_cast<const DTypeOut2*>(V_mean_lane_ptr + 8 + fv * 16);
#pragma unroll
      for (uint32_t fq = 0; fq < num_tiles_q; fq++)
      {
...
constexpr int64_t QK_TILE_CONFIG_DEFAULT = 0;
constexpr int64_t QK_TILE_CONFIG_128x32 = 1;
constexpr int64_t QK_TILE_CONFIG_64x64 = 2;
...
#define LAUNCH_QK_INT_SV_F16_KERNEL(CTA_Q_VALUE, CTA_K_VALUE, WARP_Q_VALUE, WARP_K_VALUE, DTYPE_SV_ACCUM, USE_INST_BUFFER, DENOMINATOR_ACCUM_UNIT, FUSE_V_MEAN, VALUE_MEAN_PTR) \
  do { \
    ...
    { \
      CHECK_SHAPE(query_scale, batch_size, num_qo_heads, div_ceil(qo_len, CTA_Q_LOCAL) * (CTA_Q_LOCAL / WARP_Q_LOCAL)); \
      CHECK_SHAPE(key_scale, batch_size, num_kv_heads, div_ceil(kv_len, CTA_K_LOCAL) * (CTA_K_LOCAL / WARP_K_LOCAL)); \
    } \
    else if constexpr (QK_QUANT_GRAN == static_cast<int>(QuantGranularity::kPerThread)) \
    { \
      CHECK_SHAPE(query_scale, batch_size, num_qo_heads, div_ceil(qo_len, CTA_Q_LOCAL) * (CTA_Q_LOCAL / WARP_Q_LOCAL) * 8); \
      CHECK_SHAPE(key_scale, batch_size, num_kv_heads, div_ceil(kv_len, CTA_K_LOCAL) * (CTA_K_LOCAL / WARP_K_LOCAL) * 4); \
    } \
    else \
    { \
    ...
    dim3 grid(div_ceil(qo_len, CTA_Q_LOCAL), num_qo_heads, batch_size); \
    ...
    kernel_func<<<grid, block, smem_max>>>( \
      ...
      (RETURN_LSE) ? mutable_ptr<float>(lse) : nullptr, \
...
#define LAUNCH_QK_INT_SV_F16_TILE_CONFIG(QK_TILE_CONFIG, WARP_Q_VALUE, DTYPE_SV_ACCUM, USE_INST_BUFFER, DENOMINATOR_ACCUM_UNIT, FUSE_V_MEAN, VALUE_MEAN_PTR) \
  do { \
    if ((QK_TILE_CONFIG) == QK_TILE_CONFIG_128x32) \
    { \
      if constexpr (QK_QUANT_GRAN == static_cast<int>(QuantGranularity::kPerThread) && HEAD_DIM == 128 && !IS_CAUSAL) \
      { \
        LAUNCH_QK_INT_SV_F16_KERNEL(128, 32, WARP_Q_VALUE, 32, DTYPE_SV_ACCUM, USE_INST_BUFFER, DENOMINATOR_ACCUM_UNIT, FUSE_V_MEAN, VALUE_MEAN_PTR); \
      } \
      else \
      { \
        throw std::invalid_argument("qk_tile_config=128x32 is only supported for per-thread, head_dim=128, non-causal"); \
      } \
    } \
    else if ((QK_TILE_CONFIG) == QK_TILE_CONFIG_64x64) \
    { \
      if constexpr ((HEAD_DIM == 128 || HEAD_DIM == 256) && IS_CAUSAL) \
      { \
        LAUNCH_QK_INT_SV_F16_KERNEL(64, 64, WARP_Q_VALUE, 64, DTYPE_SV_ACCUM, USE_INST_BUFFER, DENOMINATOR_ACCUM_UNIT, FUSE_V_MEAN, VALUE_MEAN_PTR); \
      } \
      else \
      { \
        throw std::invalid_argument("qk_tile_config=64x64 is only supported for head_dim 128/256 causal"); \
      } \
    } \
    else if ((QK_TILE_CONFIG) == QK_TILE_CONFIG_DEFAULT) \
    { \
      LAUNCH_QK_INT_SV_F16_KERNEL(128, 64, WARP_Q_VALUE, 64, DTYPE_SV_ACCUM, USE_INST_BUFFER, DENOMINATOR_ACCUM_UNIT, FUSE_V_MEAN, VALUE_MEAN_PTR); \
    } \
    else \
    { \
      throw std::invalid_argument("Unsupported qk_tile_config"); \
    } \
  } while (0)
                    ...
                    const int64_t return_lse)
{
  ...
  const int stride_bz_q = query.stride(0);
  const int stride_bz_k = key.stride(0);
  const int stride_bz_v = value.stride(0);
  const int stride_bz_o = output.stride(0);
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            LAUNCH_QK_INT_SV_F16_KERNEL(128, 64, 32, 64, float, false, ComputeUnit::kTensorCore, false, nullptr);
          });
        });
      });
    });
  ...
  DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
    DISPATCH_CAUSAL(is_causal, IS_CAUSAL, {
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            LAUNCH_QK_INT_SV_F16_TILE_CONFIG(qk_tile_config, 32, half, false, ComputeUnit::kTensorCore, false, nullptr);
          });
        });
      });
    });
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            LAUNCH_QK_INT_SV_F16_KERNEL(128, 64, WARP_Q, 64, float, true, ComputeUnit::kTensorCore, false, nullptr);
          });
        });
      });
    });
      ...
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            LAUNCH_QK_INT_SV_F16_KERNEL(128, 64, 32, 64, half, false, ComputeUnit::kTensorCore, true, const_ptr<DTypeOut>(value_mean));
          });
        });
      });
    });
