...
The maintained backward path targets native SM86, fp16, dense non-causal fixed-length attention with head dimension 64 and NHD/HND layouts. Its dS mirror uses one swizzled 16x64 int8 parent per N32 pair, with static 16x32 M-half views for dQ. Its only generated specialization is:
...
- Transposed dQ operand-role mirror. Evaluate `dQ^T = K^T @ dS^T` so the producer's existing `tdKdS` dK-oriented transform may feed a lane-native dS B packet. Prepack `K^T` as an A operand once per CTA, use a source-distinct pair proof before integration, and map the transposed C fragment directly into the existing internal dQ workspace contract. Promote only if the packet needs no second cross-lane transpose, replaces the eight narrow dS mirror stores or the eight dS `LDSM.x4` loads with cheaper hardware work, preserves the four-barrier/atomic contract, and wins paired end-to-end timing. Keep the public dQ layout and Triton inverse mapping unchanged.
