...
The active path beats the matched Flash backward control in all eight timing rows. These results include the new Triton preprocessing overhead and are the current backward-only end-to-end baseline; forward timing is intentionally outside this plan.
...
- Establish isolated baselines for the shared-memory stages. Keep the algorithm and the reference geometry fixed while measuring narrow variants for the P/dS mirror handoff, packed MMA-B stores/loads, dQ shared transpose, and dK/dV staging. A variant is useful only if its event timing and source counters show a reduction in the targeted traffic without moving the cost to another staging path.
