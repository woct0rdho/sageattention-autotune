...
The current D128 M64xN64 reference was measured with `smooth_k=False`, 20 warmups, and 100 repeats in two case orders per layout. It includes the retained FMA, P/dS hoist, asynchronous dO staging, and consumed-stage matrix mirror. Each row below is the ratio of geometric-mean Flash and Sage medians from the two orderings.
...
- The accepted D64 workspace swizzle, D128 P/dS hoist, D128 asynchronous dO staging repair, D128 consumed-stage matrix mirror, and D64 lane-major packet handoff remove specific on-chip costs. Their resource, counter, and candidate/control evidence is recorded under `Retained Changes` and `Closed Experiments`.
...
The exact phased packet cache, score-side K caches, and full dQ dS-mirror hoist remain closed. The consumed-stage matrix mirror resolves the bank-clean Q/dO copy screen without reopening the rejected packet cache. The remaining source-specific handoff is the half-pair P/dS exchange, which publishes only the remote half needed by each dV/dK owner and does not reopen the unstable all-slice dQ hoist.
...
- The pre-swizzle resource audit reported 243 registers per thread for tail and aligned variants, zero stack/local spill, and 57,600 bytes of dynamic shared storage. The retained pair-interleaved, lane-major-packet raw-K N128 image uses 242/239 registers and 2,176/2,008 instructions for tail/aligned, with 49,408 bytes dynamic shared memory and zero stack/local spill; smooth K uses 242/237 registers and 2,320/2,072 instructions, also with zero stack/local spill. Both retain four static barriers. N256 retains the specialization-local 128-register cap so 512 threads fit the SM86 65,536-register CTA limit. The optimized raw-K N256 image uses 86,272 bytes shared, 136/112 bytes stack, and 2,520/2,344 instructions for tail/aligned; smooth K uses the same shared allocation, 160/120 bytes stack, and 2,696/2,400 instructions. The retained D128 image after explicit FMA, P/dS hoisting, asynchronous dO staging, and consumed-stage matrix mirroring uses 78,112 bytes shared, 235/248 registers and 3,184/3,160 instructions for raw tail/aligned, with zero stack/local spill; smooth uses 232/250 registers and 3,240/3,200 instructions. All three shapes retain four static barriers and permit one resident CTA.
...
- D128 profiling-driven local cycle. Explicit outer FMA, P/dS fragment hoisting, the narrow asynchronous dO producer repair, and the consumed-stage matrix mirror are retained. CTA-wide dS-scale publication, base-2 reconstruction, every score-side K cache width, full dQ dS-mirror hoisting, the phased Q/dO packet cache, inherited-source normal LDSM, and direct-row normal LDSM are closed by resource or latency evidence. The mirror is the accepted representation for the measured transposed-B cost; the next open target is the half-pair P/dS handoff. Broader work still requires an execution tile, warp, or stage redesign.
...
- Dedicated D128 backward. QBlock=32/KBlock=64 CTA M64xN64 with eight warps is the maintained D128 reference. Each N16 tile has two M-half warps; dV/dK ownership is split between them, while two even-N pairs own four D16 dQ slices each. A 16 KiB Q/dO double buffer prevents asymmetric-tail prefetch races while preserving four barriers. The source-order quantized model matches native dQ to `2.4e-7` and dK/dV to fp16 rounding on the N65 bring-up case. It is correct and spill-free; after the consumed-stage matrix mirror, the fresh paired common baseline is `1.0241x` end to end and `1.0597x` native plus conversion against normal FlashAttention dispatch, with all eight standard rows improving.
...
The measured references are Q32/K64 D64 M64xN128/eight-warp with `smooth_k=False` and D128 M64xN64/eight-warp. The fresh common-baseline position is `1.1934x/1.2249x` D64 and `1.0241x/1.0597x` D128 for end-to-end/native-plus-conversion geometric speedup against normal FlashAttention dispatch.
...
The percentage ceilings below are planning estimates derived from measured instruction and wavefront families, not performance claims. Work in the listed order; do not start a broad D128 schedule rewrite before the half-pair handoff and bounded grouped-dQ screens are resolved.

#### D128 Changes

| Order | Change | Why it is open | Estimated ceiling | First implementation gate |
|---:|---|---|---|---|
| 1 | Half-pair P/dS C-fragment handoff | Keep each owner's local half in registers and publish only the remote P or dS half | About `2-5%` | Halve canonical P/dS stores and loads, preserve the dS mirror, prove the packet mapping, and avoid smooth-aligned spill |
| 2 | Two-D16 grouped dQ consumption | Reduce stable dS mirror copies from `32` to `16` per temporal M32 pair without the rejected four-slice lifetime | About `0-3%` | Compiler-only scoped-lifetime screen first; reject on any stack/local traffic |
| 3 | Role-specialized dQ/dKV schedule | Remove overlap between dQ reuse state and the persistent 64-register dKV accumulator image | Potentially `5-12%`, high risk | Written ownership, register, packet-byte, atomic-count, and barrier-arrival model before implementation |
| 4 | M32xN32 two-CTA schedule | Only straightforward tile class with a plausible `20%+` issue-concurrency ceiling | High risk; useful only if it reaches two CTAs | Static model and compile at `<=50 KiB` shared and `<=128` registers for eight warps or `<=170` for six |

The completed bank-clean affine search and consumed-stage mirror are retained rather than reopened. The remaining order is intentionally narrower: the half-pair handoff replaces an existing canonical exchange and transfers only one remote half; the grouped dQ experiment changes accumulator grouping and lifetime rather than retrying the rejected full hoist; role specialization and M32xN32 are schedule-level experiments that require an explicit storage and register model.
