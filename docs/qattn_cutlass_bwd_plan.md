...
- The pre-swizzle resource audit reported 243 registers per thread for tail and aligned variants, zero stack/local spill, and 57,600 bytes of dynamic shared storage. The retained pair-interleaved N128 image reports 235/234 registers for tail/aligned, 49,408 bytes dynamic shared memory, and zero stack/local spill; its `__maxnreg__` boundary remains 243. N256 needs 90,368 bytes dynamic shared memory and a specialization-local 128-register cap so 512 threads fit the SM86 65,536-register CTA limit. It emits 208/160 bytes of per-thread stack for tail/aligned and 50/62 tail plus 32/32 aligned static `LDL`/`STL` instructions. Both shapes permit only one resident CTA.
...
- Sixteen-warp feasibility. The generated N256 specialization proves that the shape is representable and runnable: 90,368 bytes of dynamic shared memory fit the device's 101,376-byte opt-in limit, and a 128-register cap fits 512 threads exactly within the SM86 CTA register limit. The first implementation spills 208/160 bytes per thread and is slower than N128 in all four initial NHD/HND sequence-4096/8192 rows. It remains available as an explicit experimental config so future work can reduce its live set against a validated baseline.
...
The next cycle uses QBlock=32, KBlock=64, CTA M64xN128, and eight warps as the measured reference, with the explicit N256 config as the architectural comparison. It should not reopen closed source-local schedules. Each candidate must state whether it changes the artifact, accuracy, ownership, scale, precision, or product contract. Preserve the current arithmetic and synchronization contract when that category is not the subject of the experiment.

### Architectural Candidates

- N256 register-lifetime reduction. The first sixteen-warp implementation already halves global dQ contributions and passes ownership, tail, and sanitizer checks, but its mandatory 128-register cap spills 208/160 bytes per thread and loses `30.0%` geometric-mean throughput relative to N128. Optimize only through a materially smaller live set or phase-specific ownership that preserves FP32 dK/dV accumulation, the pair-interleaved dS mirror, four barriers, and one dQ contribution per N256 CTA. Do not remove the cap until the uncapped allocation is at most 128 registers/thread.
