...
- The pre-swizzle resource audit reported 243 registers per thread for tail and aligned variants, zero stack/local spill, and 57,600 bytes of dynamic shared storage. The retained pair-interleaved N128 image reports 235/234 registers for tail/aligned, 49,408 bytes dynamic shared memory, and zero stack/local spill; its `__maxnreg__` boundary remains 243. N256 retains the specialization-local 128-register cap so 512 threads fit the SM86 65,536-register CTA limit. The initial N256 image needed 90,368 bytes dynamic shared memory and emitted 208/160 bytes of per-thread stack for tail/aligned. On-demand V fragments plus phased dS/dKV scratch packing reduce the retained N256 image to 86,272 bytes shared and 136/112 bytes stack. Static tail/aligned SASS falls from 2,632/2,352 to 2,520/2,344 instructions; local-memory instructions fall from 50 `LDL` and 62 `STL` to 41/36 in the tail function, and from 32/32 to 28/28 in the aligned function. Both shapes still permit only one resident CTA.
