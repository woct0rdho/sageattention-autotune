...
The public CUTLASS forward uses `per_block_int8(QBlock=32,KBlock=64)` for head dimensions 64 and 128 and returns only output and optional LSE. Its Q/K scale tensors have the same flat artifact shape required by native backward, while attention CTA and warp dimensions remain independent execution choices. Native backward still consumes only the head-64 specialization. The trainable Triton path can save per-block Q/K artifacts but does not currently route them into the native CUTLASS backward operator. No maintained forward path publishes P, P maxima, or a backward dS scale table.
...
- Forward/backward artifact compatibility. CUTLASS forward uses the same Q32/K64 Q/K scale domain at head dimensions 64 and 128, including K32 CTAs sharing one K64 scale and Q16 warps sharing one Q32 scale. Native backward consumes that contract only at head dimension 64. The forward wrapper still discards those quantized tensors after producing output/LSE, and the trainable Triton forward still does not feed saved artifacts into the native operator, so the backward-reuse benchmark remains a product-contract proxy rather than an integrated training invocation.
