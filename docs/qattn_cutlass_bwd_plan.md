...
The final `cuobjdump` SM86 resource report shows 227 registers per thread for the tail variant and 243 for the even-length variant, with zero stack and local-memory bytes. The kernel uses `__maxnreg__(243)` instead of retaining a synchronization barrier as a ptxas allocation boundary; removing the cap raised the barrier-removed tail specialization to 255 registers. `sizeof(SharedStorage2DWarp<BwdTileTraits<64, 64, 128, 8>>)` is 57,632 bytes, down 32 bytes after removing the obsolete shared dS-scale array. The final Nsight Compute capture reports 57.63 KiB dynamic shared memory, one resident block, 16.7% theoretical occupancy, and only 0.40 eligible warps per cycle on average. Register-spill traffic is zero. Any handoff optimization that increases register or shared-memory pressure must therefore demonstrate a measured kernel or end-to-end win.
