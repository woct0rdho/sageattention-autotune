...
The D128 M64xN64 schedule also has four static barriers but uses head-specific ownership. Each N16 tile has two warps, one per M16 half. The two halves exchange one common P scale, publish canonical P/dS plus the pair-interleaved dS mirror, and split final ownership so the lower-M warp accumulates dV while the upper-M warp accumulates dK. Two even-N owners each cover four D16 dQ slices and both N32 pairs. The retained dQ loop processes two adjacent D16 slices together, loads each stable dS mirror fragment once, multiplies it by two K fragments, and then converts and publishes both accumulators. Q/dO use two shared stages: the startup barrier jointly publishes K/V, packed K, and the first Q/dO pair; the inactive Q/dO stage receives the next asynchronous prefetch; and the end barrier publishes that stage. This double buffer prevents inactive tail loader warps from overwriting the current Q/dO operands while the remaining active warp pair finishes dK/dV. After P/dS reconstruction, producer warps scatter the already-correct Q and dO MMA-B words into the consumed FP16 dO stage: Q occupies its first half and dO its second half. The existing publication barrier orders normal bank-clean `LDSM.x4` consumers for dK/dV, so the mirror changes neither the two-stage tail lifetime nor the four-barrier schedule.
...
D128 maps `n_tile=warp_id/2` and `m_half=warp_id&1`, so each N16 tile has a two-warp M-half pair. The two even N tiles own the lower and upper four D16 dQ slices; each M-half owner sums both N32 pairs and writes its own M16 rows. Within an owner, adjacent D16 slices are accumulated as a bounded pair so the same two dS mirror fragments feed both slices without changing the number or order of global dQ contributions. Every configuration atomically adds one accumulated M16 fragment per CTA directly into the common global fp32 dQ workspace. The internal workspace is in MMA TV order so each warp's fragment index is contiguous in global memory.
...
The latest direct Flash-normalized D128 M64xN64 common baseline was measured with `smooth_k=False`, 20 warmups, and 100 repeats in two case orders per layout. It includes the retained FMA, P/dS hoist, asynchronous dO staging, and consumed-stage matrix mirror. The current source additionally retains two-D16 grouped dQ consumption; its native-kernel candidate/control result is recorded separately below and is not multiplied into this table. Each row below is the ratio of geometric-mean Flash and Sage medians from the two common-baseline orderings.

| Layout | Heads | Seq | Flash median ms | Sage end-to-end ms | End-to-end speedup | Sage native+convert ms | Native+convert speedup |
|---|---:|---:|---:|---:|---:|---:|---:|
| NHD | 16 | 4096 | 9.370 | 9.193 | 1.019x | 8.727 | 1.074x |
| NHD | 16 | 8192 | 35.345 | 34.408 | 1.027x | 33.703 | 1.049x |
| NHD | 32 | 4096 | 18.251 | 17.909 | 1.019x | 17.161 | 1.064x |
| NHD | 32 | 8192 | 70.146 | 68.639 | 1.022x | 67.323 | 1.042x |
| HND | 16 | 4096 | 9.731 | 9.432 | 1.032x | 8.932 | 1.089x |
| HND | 16 | 8192 | 35.823 | 34.681 | 1.033x | 34.454 | 1.040x |
| HND | 32 | 4096 | 18.442 | 17.995 | 1.025x | 17.222 | 1.071x |
| HND | 32 | 8192 | 70.711 | 69.576 | 1.016x | 67.263 | 1.051x |

This common-baseline capture wins all eight rows in both scopes. Geometric speedup is `1.0241x` end to end and `1.0597x` native plus conversion; NHD contributes `1.0219x/1.0569x` and HND contributes `1.0264x/1.0626x`. D64 N128 remains the production reference because its margin is larger, while D128 remains the dedicated maintained schedule and is no longer a Flash-performance deficit on this SM86 baseline.

The latest direct D128 raw rows are `C:/tmp/sageattn/experiments/d128_consumed_stage_matrix_mirror_20260815/flash_ratio_{nhd,hnd}_{a,b}.csv`; the retained D64 rows remain under `C:/tmp/sageattn/experiments/sagebwd_planned_opts/fresh_406a481_flash_hd64_{NHD,HND}.csv`. Candidate/control timing and profile evidence used to accept or reject individual changes is recorded under `Retained Changes` and `Closed Experiments`; those ratios are not multiplied into this Flash-normalized result.
...
- The pre-mirror D128 deficit was on-chip rather than DRAM-bound: scalar reconstruction, conversion, shared-memory issue, and fragment plumbing dominated the excess work. The retained mirror removes one measured issue family and turns that deficit into a modest Flash lead; remaining work is still on-chip.
...
- Both D128 kernels had one resident eight-warp CTA in the pre-mirror matched attribution, and the retained mirror remains one resident CTA. A new occupancy claim requires a compiled schedule with lower register and shared-memory use.
- The completed occupancy screen closes both retained-tile role specialization and the straightforward M32xN32 split. Isolated score/reconstruction/dKV and score/reconstruction/dQ paths remain above the six- and eight-warp two-CTA register ceilings, while the exact four-warp N32 storage model is above the `50 KiB` shared-memory gate and cannot increase total active warps after the reductions needed to fit.
- The accepted D64 workspace swizzle, D128 P/dS hoist, D128 asynchronous dO staging repair, D128 consumed-stage matrix mirror, D128 two-D16 grouped dQ loop, and D64 lane-major packet handoff remove specific on-chip costs. Their resource, counter, and candidate/control evidence is recorded under `Retained Changes` and `Closed Experiments`.

#### Pre-Mirror Matched D64/D128 Profile

The pre-mirror matched profile is under `C:/tmp/sageattn/experiments/d128_roofline_20260815`. It uses raw-K NHD, batch 1, sequence 4096, 16 heads, three warmups, one captured main-kernel launch, kernel replay, and the normal FlashAttention dispatch. It remains the baseline that identified the Q/dO copy cost; the retained mirror's production NCU comparison is recorded under `D128 Consumed-Stage Matrix Mirror`. NCU duration is architectural evidence only; the CUDA-event tables above remain the latency authority.
...
The pre-mirror total-instruction ratio is `2.405x` for D64 and `2.910x` for D128. D128 executes `4.65x` Flash's ALU instructions, `4.57x` its FMA-pipe instructions, `2.53x` its LSU instructions, `7.75x` its explicit shared-load instructions, and `2.64x` its shared-store instructions. The D128 MIO-throttle ratio is `1.509` stalled warps per active issue cycle versus Flash's `0.560`; D64 is `0.428` versus `0.476`. This was the clearest head-dimension split before the mirror: D64 tolerated its scalar work and won, while D128 lost tensor issue efficiency through shared/scalar plumbing. The retained mirror keeps executed instructions nearly flat but lowers MIO throttle to `0.78`, raises tensor-pipe activity to `29.63%`, and restores a Flash lead.

The pre-mirror D128 launch is not bandwidth-bound. It reads `165.10 MB` and writes `102.68 MB` in `11.39 ms`, about `23.5 GB/s`; Flash moves slightly more DRAM data. A same-process NHD phase diagnostic also places the native launch at about `94%` of D128 end-to-end time at sequence 4096 and `97%` at 8192. Preprocessing plus dQ conversion remains important for reporting scope, but the fresh paired mirror timing keeps it below the `10%` reprioritization threshold.
...
These are independent pre-mirror diagnostic medians rather than additive timing brackets, so small ordering inconsistencies are expected. They quantified why the native launch was the first target; the retained mirror subsequently closed the local Flash gap.

#### Pre-Mirror D128 Shared-Memory Attribution

The pre-mirror accepted-control sequence-8192 SourceCounters report contains `754,974,720` LDSM wavefronts versus `620,756,992` ideal, with exactly `134,217,728` excessive wavefronts (`17.778%`). SASS/source-order mapping identifies all excess as two eight-site `LDSM.16.MT88.4` families: transposed INT8 dO loads used by dV and transposed INT8 Q loads used by dK. Each site executes `2,097,152` instructions and takes `16,777,216` wavefronts versus `8,388,608` ideal. Ordinary LDSM, scalar LDS/STS, and the accepted asynchronous LDGSTS families are otherwise at or near ideal.

An isolated D128 `32 x 8` uint128 layout probe screened plain storage and seven swizzles for both ordinary MMA-A and transposed MMA-B reads. The retained `Swizzle<3,0,3>` is Pareto-best: ordinary reads take `1,024/1,024` ideal wavefronts, while transposed reads take `2,048/1,024`. Every screened layout that kept the minimum transposed penalty made ordinary reads two- or four-wave. A global Q/dO layout substitution is therefore closed. This probe identified the transposed copy path that the retained consumed-stage matrix mirror now resolves; reproducible source, CSV, and summary remain in the fresh profile directory.

#### Practical Rooflines

- Arithmetic peak is not the current limit. Flash backward performs five FP16 matrix products; Sage performs one FP16 and four INT8 products. At the SM86 dense `4:1` INT8-to-FP16 peak ratio, the ideal tensor-arithmetic floor is `2.5x` faster than five FP16 products. The compiled Sage kernels execute `0.60x` as many tensor warp instructions as Flash, which gives a more conservative `1.67x` equal-issue instruction ceiling. The pre-mirror D128 image's `2.91x` total instruction count and lower tensor utilization erased most of that advantage; the mirror improves issue efficiency without materially reducing total executed instructions.
- Matching the paper's approximately `1.25x` D128 backward ratio from the current local `1.0597x` native-plus-conversion position would require about a further `1.180x` speedup, or a `15.2%` latency reduction. Raising effective D128 tensor utilization from the mirror's `29.63%` toward the retained D64 level of `34-35%` has only part of that arithmetic magnitude and still requires lower shared and scalar work. This is a scheduling target, not a prediction.
- The consumed-stage mirror removes the repeated transposed-consumer penalty while adding one producer scatter. In production attribution, shared-load wavefronts fall `7.9%`, total shared wavefronts fall `3.6%`, and CUDA-event native-plus-conversion latency improves `7.09%` geometrically. That result closes this copy family as a retained optimization but remains only part of the route to the paper ratio.
...
- D128 canonical P/dS pair stores and owner reloads for dV/dK. A source-distinct half-pair candidate targeted only this exchange, but its register-conversion overhead regressed latency, so the full canonical exchange remains active.
...
- D64 packed Q/dO fragment stores and loads. D128 producer warps still read Q/dO through the retained transposed source path, then scatter the MMA-B fragments into the consumed-stage matrix mirror; dK/dV consumers use normal bank-clean `LDSM.x4` loads.
...
Per temporal D128 M32 pair, the canonical P/dS exchange logically stores about `4 KiB` and reloads about `4 KiB`: four N16 tiles each publish `512 B` of P and `512 B` of dS, then four dV owners load one P pair and four dK owners load one dS pair. The tested half-pair formulation kept the local C-fragment half in registers, published only the remote `256 B` half per producer, and reduced both logical sides to about `2 KiB` while leaving the dS mirror intact for dQ. Its 64-seed byte proof passed, but the production conversion added `56-72` static instructions and event timing regressed to `0.982055x`; the current source therefore retains the full canonical exchange.
...
| Q/dO packet and next-pair traffic | D64 packet handoff and the D128 consumed-stage matrix mirror are retained; early-dO and tail-prefetch variants regressed | The mirror removes the recurring transposed-consumer penalty with one producer scatter and normal bank-clean `LDSM.x4` reloads, while the accepted narrow producer-layout repair removes recurrent `LDGSTS` excess without changing the publication barrier or tail safety |
...
The exact phased packet cache, score-side K caches, full dQ dS-mirror hoist, and half-pair P/dS exchange remain closed. The consumed-stage matrix mirror resolves the bank-clean Q/dO copy screen without reopening the rejected packet cache. The bounded two-D16 dQ loop is retained because it amortizes the same stable dS fragments across two adjacent K slices without taking the unstable four-slice lifetime. Compiler-isolated role specialization and the exact M32xN32 storage model now close the remaining schedule-level occupancy forms under the retained shared-memory, register, barrier, and packet contracts.
...
- The D128 consumed-stage matrix mirror now removes the measured transposed-Q/dO shared penalty without changing the ordinary consumer layout, shared storage, barrier count, or two-stage tail lifetime. The affine remap and direct-row register paths are closed; any future copy-path work needs a materially different ownership representation.
- The D128 half-pair P/dS handoff is closed. Its hybrid local-C/remote-`LDSM.x2` fragments were byte-exact over 64 deterministic seeds, but the production image added MOVM, SHFL, PRMT, and control-flow work and regressed the raw-NHD S4096/H16 event bracket to `0.982055x`.
- The D128 two-D16 grouped dQ loop is retained. Across the four dQ owners it reduces stable dS `LDSM.x4` copies from `32` to `16` per temporal M32 pair, lowers executed LDSM by `3.922%`, and wins all 16 raw/smooth NHD/HND H16/H32 S4096/S8192 native-kernel rows at `1.012676x` geometrically. It does so with two live dQ accumulators, raising aligned raw/smooth allocation to `254` registers without spill.
- Explicit M64xN64 role specialization is closed at the compiler/storage gate. With dQ compiled out, the score/reconstruction/persistent-dKV path still uses raw `217/223` and smooth `215/223` tail/aligned registers. With dKV compiled out, the complementary path still uses raw `208/200` and smooth `156/198`. Both exceed the approximately `170`-register twelve-warp ceiling before role-transfer code, while reducing overlap in the current eight-warp CTA cannot add residency because shared storage remains `78,112` bytes. Parking the four overlapping dKV fragments requires `32 KiB`, versus only about `20 KiB` of phase-dead Q/dO and score storage.
- The straightforward M32xN32 two-CTA form is also closed at the static gate. An exact `BwdTileTraits<128,32,32,4>` storage probe reports `55,568` bytes per current-style CTA, or `111,136` bytes for two blocks. Six- and eight-warp forms inherit isolated path demands above their `170`- and `128`-register ceilings. Storage-reduced four-warp forms fit only by removing packed K together with another retained representation or by single-buffering FP16 dO; they expose the same eight total resident warps while doubling Q/dO traversal and dQ contributions relative to N64.
- M64xN32 and M32xN64 remain one-CTA shapes and add replication or loop boundaries. M64xN128 exceeds the shared-memory budget without phasing and doubles dK/dV ownership pressure. The retained M64xN64/eight-warp schedule is therefore the D128 endpoint under the fixed artifact, precision, workspace, and synchronization contract.
...
- The pre-swizzle resource audit reported 243 registers per thread for tail and aligned variants, zero stack/local spill, and 57,600 bytes of dynamic shared storage. The retained pair-interleaved, lane-major-packet raw-K N128 image uses 242/239 registers and 2,176/2,008 instructions for tail/aligned, with 49,408 bytes dynamic shared memory and zero stack/local spill; smooth K uses 242/237 registers and 2,320/2,072 instructions, also with zero stack/local spill. Both retain four static barriers. N256 retains the specialization-local 128-register cap so 512 threads fit the SM86 65,536-register CTA limit. The optimized raw-K N256 image uses 86,272 bytes shared, 136/112 bytes stack, and 2,520/2,344 instructions for tail/aligned; smooth K uses the same shared allocation, 160/120 bytes stack, and 2,696/2,400 instructions. The retained D128 image after explicit FMA, P/dS hoisting, asynchronous dO staging, consumed-stage matrix mirroring, and two-D16 grouped dQ uses 78,112 bytes shared, 235/254 registers and 3,176/3,152 instructions for raw tail/aligned, with zero stack/local spill; smooth uses 232/254 registers and 3,232/3,192 instructions. All three shapes retain four static barriers and permit one resident CTA.
...
- D128 profiling-driven cycle. Explicit outer FMA, P/dS fragment hoisting, the narrow asynchronous dO producer repair, the consumed-stage matrix mirror, and two-D16 grouped dQ are retained. CTA-wide dS-scale publication, base-2 reconstruction, every score-side K cache width, full dQ dS-mirror hoisting, the phased Q/dO packet cache, half-pair P/dS handoff, inherited-source normal LDSM, and direct-row normal LDSM are closed by resource or latency evidence. Compiler-isolated role specialization and the exact M32xN32 storage model also close the schedule-level occupancy branch. The mirror is the accepted representation for the measured transposed-B cost, bounded grouping is the accepted dS-load amortization, and M64xN64/eight-warps remains the endpoint under the retained contract.
...
- D128 inherited-source normal-LDSM repack. An eight-seed source-distinct probe derived an exact normal-LDSM-to-MMA-B network, but the first production atom inherited the transposed copy's source value layout. It therefore retained the same bank-conflicting lane addresses, added `120` static instructions per D128 image, and left matched S4096 load conflicts/wavefronts effectively unchanged at `33.9M/212.4M`. This exact representation is closed. A further normal-LDSM experiment must begin from the conflict-free ordinary source partition rather than changing only the hardware opcode and destination repack.
- D128 direct-row normal-LDSM repack. Addressing physical Q/dO row `lane_id` makes the normal x4 load bank-clean and removes the full measured transposed-load excess, but the exact MMA-B conversion requires three mixed packed-register/lane exchanges. The production image added `192 MOVM` and `64 PRMT` instructions, and an event-timed NHD ABBA bracket regressed all four rows to `0.8367x` geometrically. This direct-row representation is closed. Triton's linear-layout decomposition confirms that the local conversion is intrinsically multi-stage; any remaining register path must jointly choose a different bank-clean lane-address map and fragment ownership.
...
- D64 Q/dO residual cycle. Matched aligned S8192 NHD/HND SourceCounters isolate the retained preparation cost from global-source effects. Both layouts have exactly `8,388,608` excessive wavefronts at the two `LDSM.16.MT88.4` packet-preparation sites. NHD also has `4,259,840` `LDGSTS` excess, including `4,177,920` recurrent wavefronts, while HND has only `65,536` startup excess and zero recurrent excess. The ordinary-source map is the same mixed lane/register/byte permutation already derived for D128. Its exact minimum-stage repack removes the full preparation excess in both layouts, but adds `24 MOVM`, eight `PRMT`, and `24-32` static instructions per specialization and regresses the governing NHD CUDA-event ABBA result to `0.9644x` with zero of four wins. The lane-major packet handoff and retained `Swizzle<3,0,3>` are therefore the D64 endpoint.
- N256 compiler-isolation cycle. With the register cap relaxed, full raw/smooth images need `231-252` registers; deleting dQ still needs `194-211`, and deleting dK/dV still needs `196-239`. At the mandatory 128-register cap, those isolated halves retain `72-128` and `24-64` bytes of stack. A single dK-only or dV-only path is approximately cap-clean, proving the ownership boundary: sixteen N16 tiles require 32 persistent dK/dV roles across only 16 warps. Parking the missing 16 FP32 fragments requires `65,536` bytes, while exact N256 shared storage is already `86,272` of the device's `101,376` opt-in bytes. CUDA shared spilling is unavailable for this dynamic-shared kernel, and `launch_bounds(512,1)` reproduces the same stack frames. The current N256 accumulator/ownership representation is closed before production timing.
...
- Dedicated D128 backward. QBlock=32/KBlock=64 CTA M64xN64 with eight warps is the maintained D128 reference. Each N16 tile has two M-half warps; dV/dK ownership is split between them, while two even-N pairs own four D16 dQ slices each. A 16 KiB Q/dO double buffer prevents asymmetric-tail prefetch races while preserving four barriers. The source-order quantized model matches native dQ to `2.4e-7` and dK/dV to fp16 rounding on the N65 bring-up case. It is correct and spill-free. The latest direct Flash-normalized common baseline, captured after the consumed-stage matrix mirror and before grouped dQ, is `1.0241x` end to end and `1.0597x` native plus conversion with all eight standard rows improving; the current grouped-dQ source has a separate `1.012676x` native-kernel candidate/control result.
...
- D128 two-D16 grouped dQ consumption. Each dQ owner loads one stable dS mirror fragment and applies it to two adjacent K D16 fragments, holding two bounded accumulators before conversion and publication. Raw tail/aligned resources are `235/254` registers and `3,176/3,152` instructions; smooth uses `232/254` and `3,232/3,192`, with zero stack/local traffic, unchanged `78,112`-byte shared storage, and four barriers. Static LDSM falls from `64` to `60`, all eight D64 N128/N256 machine streams remain exact, and matched NCU reports `3.922%` fewer executed LDSM operations, `4.176%` fewer shared-load wavefronts, and `6.721%` lower MIO throttle. Native-kernel graph-replay ABBA wins all 16 raw/smooth NHD/HND H16/H32 S4096/S8192 rows at `1.012676x` geometrically. Focused and full tests, direct output comparisons, long accuracy, Racecheck, and Synccheck all pass.
...
#### D128 Consumed-Stage Matrix Mirror

The D128 copy-path search was completed before production integration. Physical Q/dO row offsets cycle through eight bank classes, and all 12 simple low/high row-bit permutations retain three mixed lane/register bits. A broader screen of `64,512` bank-clean affine maps found only mixed-rank-three or mixed-rank-four ownership. The CuTe/Triton block-rank proof closes the full affine family: bank cleanliness forces the source address to carry the three independent target bits for the row group, so at least three target register bits must cross into source lane ownership. Direct-row ownership already attains that lower bound and cannot become a cheaper register path through lane-address remapping.

The retained representation aliases the first and second halves of each consumed `shared.dO_fp16_pair` stage as Q and dO matrix mirrors. Each producer still performs the required transposed source `LDSM` once, then scatters its four already-correct MMA-B words to the bank-clean matrix coordinates `row=(lane_id >> 2) + 8*word` and `column=4*(lane_id & 3)`. The existing pre-dK/dV CTA barrier publishes the mirror. dK/dV consumers use ordinary `LDSM.x4` loads, so the repeated transposed-B address partition is removed without changing shared storage (`78,112` bytes), the two-stage tail lifetime, or the four static CTA barriers. A source-distinct 64-seed proof reports `mismatches=0` over `8,192` words; independent mirror store and load probes report zero conflicts and `1,024` wavefronts for `1,024` warp operations. The per-slice `__syncwarp()` is a compiler scheduling fence; it emits no additional `WARPSYNC` or CTA barrier.

The retained compiler gate is clean. Relative to the accepted asynchronous-staging control, raw tail/aligned resources change from `241/252` registers and `3,248/3,224` instructions to `235/248` and `3,184/3,160`; smooth changes from `242/254` and `3,312/3,256` to `232/250` and `3,240/3,200`. Stack/local traffic remains zero, shared storage remains `78,112` bytes, all four barriers remain, and all eight D64 N128/N256 machine-code sequences are instruction-for-instruction identical. The conversion network is largely removed: `SHFL` falls from `129/129` raw and `134/133` smooth tail/aligned to `16/16` and `21/20`, while `PRMT` falls from `116/100` to `60/44` in every corresponding image. The two producer transposed loads account for the expected `LDSM` increase from `51,380,224` to `53,477,376` executed operations.

Matched production NCU captures at the raw aligned S4096 attribution point report:

| Metric | Control | Mirror | Change |
|---|---:|---:|---:|
| Duration | `11.389 ms` | `10.412 ms` | `1.0938x` |
| Executed instructions | `811,035,648` | `811,527,168` | `+0.06%` |
| Shared-load conflicts | `33,901,971` | `8,679,841` | `-74.4%` |
| Shared-load wavefronts | `212,422,035` | `195,588,513` | `-7.9%` |
| Shared-store wavefronts | `22,621,238` | `31,022,402` | `+37.1%` |
| Total shared wavefronts | `235,043,273` | `226,610,915` | `-3.6%` |
| Tensor-pipe active | `27.09%` | `29.63%` | higher |
| MIO-throttle ratio | `1.51` | `0.78` | lower |

The added bank-clean scatter increases stores, but the net shared wavefront count and issue pressure both fall. CUDA-event ABBA promotion uses native launch plus dQ conversion, excludes Q/K preparation, and aggregates two orders with four medians per row:

| Layout | H16/S4096 | H16/S8192 | H32/S4096 | H32/S8192 |
|---|---:|---:|---:|---:|
| NHD | `1.0807x` | `1.0696x` | `1.0645x` | `1.0696x` |
| HND | `1.0739x` | `1.0716x` | `1.0658x` | `1.0713x` |

All eight rows improve; the combined geometric mean is `1.0709x` with eight of eight wins. A separate paired common-baseline run against normal FlashAttention also wins all eight rows: NHD/HND geometric speedups are `1.0219x/1.0264x` end to end and `1.0569x/1.0626x` native plus conversion, or `1.0241x/1.0597x` combined. The focused 180-case CUTLASS suite passes. Direct NHD/HND sequence-1024/1025 raw/smooth captures keep dK/dV bitwise identical and bound cross-run dQ differences to `1.22e-4` absolute and `8.35e-6` relative norm. Long S4096/S8192 accuracy remains at the known fixed-Q32/K64 D128 floor. The complete D128 Racecheck/Synccheck matrix at NHD/HND sequence 64/65 raw/smooth reports zero hazards and zero errors. The complete `2,659`-node test suite passes with eight pytest workers. Two unrelated random-input forward absolute-error bounds are now head-dimension or dtype specific after repeated failures where relative errors remained inside their existing gates; no backward tolerance changed. The mirror is retained; the inherited-source and direct-row normal-LDSM representations remain closed.

#### D128 Half-Pair P/dS Handoff

The half-pair candidate kept each producer's local M16 C-fragment half in registers and published only the remote half required by the opposite dV or dK owner. The local half used the existing C-to-A conversion restricted to `Half`, while the remote half used `ldmatrix.sync.aligned.x2.m8n8.shared.b16`. The pair-interleaved dS mirror for dQ, Q32/K64 artifacts, dK/dV ownership, Q/dO double buffer, shared allocation, and four barriers were unchanged. A source-distinct GPU probe covered 64 deterministic seeds and compared `65,536` bytes across the hybrid P and dS fragments with zero mismatches.

The production compiler gate remained spill-free and left all eight D64 N128/N256 functions word-for-word identical. Relative to the matrix-mirror control, raw tail/aligned registers changed `235/248 -> 233/248` and smooth changed `232/250 -> 231/250`; shared storage remained `78,112` bytes and the static barrier count remained four. The intended traffic reduction did not become a cheaper instruction stream: static SASS increased by `56/56` raw tail/aligned and `72/64` smooth, with new MOVM, SHFL, PRMT, and branch work around the hybrid-fragment construction.

The focused 52-case suite passed. Direct NHD/HND sequence-1024/1025 raw/smooth captures kept dK/dV bitwise identical and bounded dQ differences to `1.22e-4` absolute and `9.2e-6` relative norm. The raw-NHD B1/S4096/H16 CUDA-event ABBA gate rejected the candidate: control and candidate geometric medians were `8.761208` and `8.921299 ms`, or `0.982055x`. The source and matrix-mirror binary were restored before the next experiment. The exact half-pair representation is closed.

#### D128 Two-D16 Grouped dQ Consumption

The retained grouped loop advances each owner's four D16 output slices two at a time. For each N32 pair it loads the stable dS mirror fragment once, loads two adjacent packed-K MMA-B fragments, and accumulates into two independent dQ fragments. Both fragments are then converted with the same fixed scale and published through the unchanged swizzled FP32 dQ workspace. This preserves source-order accumulation within each D16 slice, global atomic count and ordering class, pair-interleaved dS, Q/dO stages, shared storage, four barriers, and all public artifacts.

The compiler gate is clean. Relative to the matrix-mirror control, raw tail/aligned resources change from `235/248` registers and `3,184/3,160` instructions to `235/254` and `3,176/3,152`; smooth changes from `232/250` and `3,240/3,200` to `232/254` and `3,232/3,192`. Static LDSM falls from `64` to `60` in every image. Stack/local traffic remains zero, shared storage remains `78,112` bytes, all four barriers remain, and all eight D64 N128/N256 machine-code sequences are exact against control. The aligned image allocates the full 256-register granule at launch, so further role or lifetime work must treat `254` registers as the active ceiling rather than the pre-grouping `248/250` state.

Matched raw-NHD B1/S4096/H16 NCU attribution uses three warmups, one captured D128 launch, kernel replay, and four metric passes. Executed instructions fall `0.452%`, executed LDSM falls `3.922%`, shared-load wavefronts fall `4.176%`, and MIO throttle falls `6.721%`. Shared-load bank conflicts rise `2.536%`, tensor activity is effectively flat, and local traffic remains zero. NCU duration moves `10.411520 -> 10.424160 ms` (`+0.121%`); this is attribution evidence only and is not the latency decision.

Native-kernel CUDA-event graph-replay ABBA covers NHD/HND, raw/smooth K, H16/H32, and S4096/S8192. All 16 rows improve, spanning `1.004719-1.023968x`; subset geometric means are NHD raw `1.012133x`, HND raw `1.009647x`, NHD smooth `1.012987x`, and HND smooth `1.015947x`, for `1.012676x` combined. Direct sequence-1024/1025 captures keep dK/dV bitwise identical and bound dQ differences to `1.22e-4` absolute and `8.1e-6` relative norm. Long S4096/S8192 NHD/HND raw/smooth accuracy remains at the established fixed-Q32/K64 D128 floor. The focused CUTLASS collection passes `183` tests, all 16 Racecheck/Synccheck cases report zero hazards or errors, and the full eight-worker repository gate passes `2,659` tests. Artifacts are under `C:/tmp/sageattn/experiments/d128_two_d16_dq_20260815`.

#### D128 M64xN64 Role Specialization

The schedule-level gate first compiled complementary lower-bound images from the exact grouped-dQ source. Removing the complete dQ block leaves score reconstruction and persistent dKV at raw tail/aligned `217/223` and smooth `215/223` registers. Removing persistent dKV accumulation and stores leaves score reconstruction plus grouped dQ at raw `208/200` and smooth `156/198`. Every image has zero stack/local traffic. A twelve-warp CTA can use only about `170` registers per thread on the SM86 65,536-register file, so even the isolated paths fail before adding packet transfer, additional ownership, or synchronization. A sixteen-warp form is limited to `128` registers and fails more decisively.

An eight-warp role split cannot change residency because the retained CTA already consumes `78,112` bytes of shared memory. Each persistent dKV fragment is 64 FP32 values per lane, or `8 KiB` per warp. Parking all four fragments that overlap dQ therefore needs `32 KiB`; after dKV consumption, only the current `16 KiB` Q/dO stage and `4 KiB` canonical-score region are dead. Resident V cannot supply the missing space without reloading its full `16 KiB` tile on every temporal M32 iteration. Parking only two fragments adds `16 KiB` stores and `16 KiB` loads per iteration, halves active dQ warps, and crosses no occupancy threshold. The accepted source and extension were restored byte-for-byte after diagnostics; artifacts are under `C:/tmp/sageattn/experiments/d128_role_specialization_20260815`. This M64xN64 role-specialization class is closed.

#### D128 M32xN32 Two-CTA Schedule

An exact host layout probe instantiated `BwdTileTraits<128,32,32,4>` and right-sized K, V, packed K, canonical score, and dS/dKV scratch for an N32 CTA. The current double-buffered representation uses `55,568` bytes: Q INT8 `8,192`, dO INT8 `8,192`, K `4,096`, packed K `4,096`, dO FP16 `16,384`, scratch `4,096`, score `2,048`, V `8,192`, and row state/P scales `272`. Two blocks would require `111,136` bytes, above the approximately `100 KiB` SM86 budget and the `50 KiB` per-CTA gate.

Six- and eight-warp two-CTA forms fail the isolated `170`- and `128`-register ceilings above. Four warps can fit the register file, but two four-warp CTAs expose only eight active warps, equal to the retained block. Fitting layouts require a contract cost: single-buffering FP16 dO reaches `47,376` bytes but needs a new publication boundary or duplicate loads; removing packed K plus compact scratch reaches `49,440` bytes; removing packed K plus the rejected half-score representation reaches `50,464` bytes. The packed-K variants repeat K conversion every M32 iteration and the compact scratch reopens a bank-conflicting control. Independently, halving N doubles grid-wide Q/dO traversal and full-MxD FP32 dQ contributions: at S4096/D128 each rises from about `128 MiB` to `256 MiB` per head before atomic read-modify-write traffic. No fitting form adds resident warps or supplies a source-attributed mechanism strong enough to offset those costs, so no production compile or timing run is warranted. Artifacts are under `C:/tmp/sageattn/experiments/d128_m32n32_model_20260815`; the straightforward two-CTA schedule is closed.

#### D128 Inherited-Source Normal-LDSM Repack

The first non-transposed-copy experiment isolated the register transformation before production integration. Eight source-distinct GPU seeds mapped all `512` target MMA-B bytes uniquely to the `512` bytes returned by `SM75_U32x4_LDSM_N`. The synthesized network was two `MOVM` stages around one warp-lane permutation, with byte permutations before and after the second `MOVM`; the standalone MMA probe compared `8,192` output words with zero mismatches. Per transposed-B copy, the source expression required eight `MOVM`, four `SHFL`, and eight `PRMT` instructions.

The production atom intentionally changed only D128 and inherited the accepted transposed atom's CuTe source value layout. The compiler screen remained spill-free with `78,112` bytes shared and four barriers. Raw tail/aligned registers changed from `241/252` to `234/252`, smooth from `242/254` to `237/254`, and all eight D64 N128/N256 functions remained instruction-for-instruction identical. Static D128 instructions nevertheless rose by `120` in every image: raw `3,248/3,224 -> 3,368/3,344` and smooth `3,312/3,256 -> 3,432/3,384`. Each image added `128` `MOVM`, replaced `64` existing `SHFL`, and added `64` net `PRMT`; the `62` static LDSM instructions were unchanged.

Focused correctness passed all `180` CUTLASS cases. Direct NHD/HND raw/smooth captures at sequence 1024/1025 kept dK/dV bitwise identical and bounded cross-run dQ atomic-order differences to `1.22e-4` and `8.04e-6` relative norm. The required attribution gate rejected the representation before sanitizers or CUDA-event timing. In matched raw-aligned S4096 NCU captures, control/candidate shared-load conflicts were `33,894,734/33,911,798` and wavefronts were `212,414,798/212,431,862`; `LDSM` execution was identical at `51,380,224`. Total executed instructions rose from `811,035,648` to `873,950,208`, tensor-pipe utilization fell from `27.08%` to `23.96%`, and NCU duration rose from `11.396` to `12.882 ms`. The normal opcode did not remove the target cost because the inherited source layout still supplied the transposed lane-address partition. This exact same-source atom is closed; the source, accepted binary, and accepted SHA256 were restored. The remaining register-copy path must derive a new exact mapping from the bank-clean ordinary source addresses, not reuse this network unchanged.

#### D128 Direct-Row Normal-LDSM Repack

The second copy-path experiment changed both the shared source addresses and the register conversion. Each lane supplied the address of physical row `lane_id`, column zero in the retained `Swizzle<3,0,3>` Q/dO stage. An isolated 256-load probe measured the current transposed-B copy at `1,024` conflicts and `2,048` wavefronts versus the `1,024` ideal, while the direct-row normal x4 load measured zero conflicts and `1,024/1,024` wavefronts. Eight source-distinct seeds then mapped all `512` target MMA-B bytes uniquely to the `512` direct-row source bytes. The coordinate map is a pure GF(2) permutation, and a 64-seed implementation compared `8,192` words with zero mismatches.

The shortest uniform hardware network found for that map is `MOVM -> lane -> local -> MOVM -> lane -> local -> MOVM`: twelve `MOVM`, eight `SHFL`, and eight `PRMT` instructions per MMA-B load. Triton's current warp-layout lowering reaches the same structural conclusion from linear-layout algebra. The packed conversion contains three mixed register/lane cycles, including two byte-packed register bits; its generic lowering requires multiple pre-shuffle and post-shuffle permutation stages rather than one cheap lane exchange. This is a property of the direct-row fragment ownership, not a missing CuTe syntax shortcut.

The D128-only production compiler screen remained spill-free with `78,112` bytes shared, four barriers, and all eight D64 N128/N256 functions instruction-identical to control. Raw/smooth tail/aligned images each grew by `256` static instructions: raw `3,248/3,224 -> 3,504/3,480` and smooth `3,312/3,256 -> 3,568/3,520`. Every image added `192 MOVM` and `64` net `PRMT`; `SHFL`, `LDSM`, and the tensor instruction counts were unchanged. D128 registers were `254/252` raw and `254/254` smooth, with zero stack/local traffic.

The focused `180`-case CUTLASS suite passed. Direct NHD/HND raw/smooth captures at sequence 1024/1025 kept dK/dV bitwise identical and bounded cross-run dQ differences to `6.10e-5` and `8.56e-6` relative norm. Matched raw-aligned S4096 attribution proved that the intended hardware cost was removed: shared-load conflicts fell from `33,889,256` to `245,424`, wavefronts fell from `212,409,320` to `178,765,488`, and LDSM execution remained `51,380,224`. The conversion cost dominated: total executed instructions rose from `811,035,648` to `948,349,952`, tensor utilization fell from `27.08%` to `21.79%`, and MIO-throttle stalls increased. A 10-warmup/50-repeat two-order NHD CUDA-event ABBA bracket rejected the candidate on all four standard rows: H16/S4096 `0.8281x`, H16/S8192 `0.8439x`, H32/S4096 `0.8363x`, and H32/S8192 `0.8386x`, or `0.8367x` geometrically. Sanitizers and HND timing were skipped after the decisive event result. The source and accepted binary were restored.

Both simple endpoints are now closed: retaining the transposed source partition preserves the bank penalty, while direct physical-row ownership makes the exact register transpose too expensive. The completed affine search screened `64,512` bank-clean maps and found only mixed-rank-three or mixed-rank-four ownership; the CuTe/Triton linear-layout proof shows that every bank-clean affine map must carry at least three independent target register bits into source lane ownership. Direct-row ownership already meets that lower bound, so no affine lane-address remap can make its conversion cheaper. The consumed-stage matrix mirror therefore becomes the retained representation: producer scatter plus normal bank-clean consumer loads, with no additional storage or barrier.
...
Two 20-warmup/100-repeat CUDA-event ABBA brackets per layout improve every standard D64 row. NHD H16/S4096, H16/S8192, H32/S4096, and H32/S8192 improve by `1.0513x`, `1.0361x`, `1.0277x`, and `1.0358x`; HND improves by `1.0420x`, `1.0304x`, `1.0278x`, and `1.0367x`. The combined geometric mean is `1.0360x` with eight of eight wins. The complete 2,659-test collection passes through bounded partitions; one unrelated BF16 causal forward tolerance case passed on exact rerun. The D64 N128-only lane-major packet layout is retained.

#### D64 N128 Ordinary-Source Q/dO Repack

The final residual experiment compared the same retained aligned kernel across NHD and HND before changing any shared source. At `B=1,H=16,S=8192,D=64`, NHD has `316,080,128/303,431,680/12,648,448` observed/ideal/excessive shared wavefronts. HND has `311,885,824/303,431,680/8,454,144`. Both layouts attribute exactly `8,388,608` excess to the two Q/dO packet-preparation `LDSM.16.MT88.4` instructions. The remaining `LDGSTS` excess is source-layout dependent: NHD has `4,177,920` recurrent plus `81,920` startup wavefronts, while HND has zero recurrent and `65,536` startup wavefronts. Packet stores, lane-major `LDS.128` consumers, ordinary score loads, and all shared stores remain at their ideal count.

A source-distinct byte probe derives `source_bits_from_target_bits=(4,8,5,6,0,2,3,1,7)`, or `sl0=l4, sl1=r1, sl2=b0, sl3=b1, sl4=l0, sb0=l2, sb1=l3, sr0=l1, sr1=r0`. This is exactly the D128 ordinary-source map. Exhaustive search over MOVM, lane-bit permutations, and local byte/register permutations proves a minimum seven-stage network: three MOVM stages, two lane exchanges, and two local permutations. The integrated D64 candidate used an ordinary bank-clean `LDSM.x4` followed by that exact repack. All 52 focused cases passed; direct NHD/HND S1024/S1025/S4096/S4097 raw/smooth captures kept dK/dV bitwise identical and bounded dQ differences to `1.22e-4` and less than `9e-6` relative norm.

Resources remained raw `242/239` and smooth `242/237` registers for tail/aligned images, with zero stack/local, `49,408` bytes shared, and four barriers. Static SASS grew from raw `2,176/2,008` to `2,208/2,032` and smooth `2,320/2,072` to `2,352/2,104`; every image adds 24 MOVM and eight PRMT instructions. SourceCounters proves the intended removal: candidate NHD falls to `4,259,840` excess and HND to `65,536`, exactly `8,388,608` below their controls. The instruction cost dominates. A two-order 20-warmup/100-repeat NHD CUDA-event ABBA bracket reports `0.9611x`, `0.9611x`, `0.9652x`, and `0.9704x` at H16/H32 and S4096/S8192, or `0.9644x` geometrically with zero wins. HND timing and sanitizers were skipped after the decisive event rejection. The candidate is removed; the retained lane-major handoff is the endpoint for this residual.
...
#### D64 N256 Compiler-Isolated Residency Bound

Compile-only deletion variants establish a kernel-wide lower bound. Entries below are raw-tail, raw-aligned, smooth-tail, and smooth-aligned. Every relaxed-cap image has zero stack:

| Isolated N256 path | Unrestricted registers | Stack bytes at 128 registers |
|---|---|---|
| Full kernel | `231 / 252 / 242 / 252` | `136 / 112 / 160 / 120` |
| Score plus dK/dV, no dQ | `194 / 211 / 209 / 204` | `128 / 72 / 120 / 96` |
| Score plus dQ, no dK/dV | `196 / 239 / 198 / 223` | `40 / 24 / 48 / 64` |
| Score only | `167 / 127 / 168 / 119` | `0 / 0 / 0 / 0` |
| Score plus dV only | `161 / 168 / 163 / 165` | `0 / 0 / 0 / 8` |
| Score plus dK only | `164 / 157 / 171 / 148` | `8 / 0 / 0 / 0` |

The single-output rows show that one dK or dV fragment can nearly fit, but N256 needs both outputs for each of its sixteen N16 tiles. This is 32 persistent logical output roles for 16 physical warps, so a no-storage schedule must assign at least two 32-float-per-thread FP32 fragments to one warp. Giving every warp only one role requires parking the other sixteen fragments, exactly `16 warps * 32 lanes * 32 floats * 4 bytes = 65,536` bytes. N256 already uses `86,272` dynamic shared bytes and the local SM86 device permits `101,376`, leaving `15,104`; the parked representation cannot reside. A 32-warp CTA would have only 64 registers per thread from the same 65,536-register file, below even the aligned score-only requirement.

This is active traffic rather than a dormant frame artifact. The aligned raw cap-128 kernel at B1/H16/S4096 executes `325,074,944` local-load and `20,316,160` local-store thread instructions, covering `45,287,424` local L2 sectors. Replacing `__maxnreg__` with `launch_bounds(512,1)` produces identical full-kernel stack sizes. CUDA 13 shared-memory spilling is not an escape hatch: ptxas rejects it with `Pragma 'enable_smem_spilling' is not allowed for dynamic SMEM`. The disabled-output objects are compiler diagnostics only and were never launched. Straightforward N256 role specialization and source-local lifetime tuning are closed; a future candidate must change the accumulator count or external storage/workspace contract.
...
Correctness, sanitizer, and timing work were skipped exactly as required by the compiler-first gate. The candidate source was reverted, the three stable generated units were restored, and the accepted extension was restored. The candidate binary, source patch, resources, and SASS are retained under `C:/tmp/sageattn/experiments/d64_split_dkv_n128_nw16`. This closes D64 schedule tuning under the current SM86, Q32/K64, public-layout, and four-barrier contract.
...
The later ordinary-source repack closes the residual more directly. It removes the same `8,388,608` preparation wavefronts from both NHD and HND, proving that those sites are real and separable, while the HND comparison proves that the recurrent NHD `LDGSTS` excess is global-source-layout induced. Its exact seven-stage mixed register/lane conversion still regresses NHD event timing to `0.9644x`. The attribution is therefore complete: neither the producer-source layout nor the retained lane-major packet consumer justifies a production change.
...
- Keep the D64 N128 ordinary-source Q/dO repack closed. It removes the full measured preparation-LDSM excess, but its minimum mixed register/lane network adds 24 MOVM and eight PRMT instructions and regresses the governing event result to `0.9644x`. The retained lane-major stores and consumers are already bank-clean.
...
- Keep the current D128 Q/dO `Swizzle<3,0,3>` for the ordinary stage. The screened plain and alternate swizzles cannot remove the transposed-B penalty without worsening ordinary loads. Both simple normal-LDSM endpoints are closed: inherited transposed addresses retain the conflict, and direct row addresses require a prohibitive three-stage mixed register/lane transpose. The affine lower-bound proof closes the remaining remap family; the consumed-stage matrix mirror is retained as the bank-clean representation and must preserve its existing alias, barrier, and tail-lifetime contract.
- Keep M64xN64/eight-warps as the D128 schedule endpoint under the retained contract. Twelve- and sixteen-warp role-specialized forms exceed their register ceilings even with dQ or dKV removed, while role parking in the current CTA exceeds phase-dead shared storage or adds traffic without increasing residency.
- Keep the straightforward M32xN32 split closed. Its current-style four-warp storage is `55,568` bytes, and the fitting variants remove packed K, reopen bank-conflicting or rejected handoffs, or disturb the four-barrier double-buffer lifetime. Two four-warp CTAs do not increase active warps and double Q/dO traversal plus dQ contributions.
...
- Use the retained D64 producer-register handoff and swizzled global dQAccum workspace as the native local baseline. Do not restore D64 canonical P/dS score-pair stores or treat its removed slots as free storage. D128 retains full canonical P/dS score-pair storage; the exact hybrid local-half/remote-half replacement is closed by its `0.982055x` latency result.
...
- Keep N256's 128-register cap. The current 512-thread CTA cannot request 129 registers; the full kernel needs `231-252` unrestricted registers, and the isolated score-plus-dQ and score-plus-dKV halves independently need `196-239` and `194-211`. Sixteen additional FP32 output fragments require `65,536` bytes, far above the `15,104` bytes of shared headroom. Reopen N256 only with a representation that changes this accumulator/storage equation, compiles at no more than 128 registers with zero stack/local traffic, and beats both the retained experimental N256 image and unchanged N128.
...
No source-supported production change remains under the retained Q32/K64, workspace, precision, and four-barrier contract. The completed residual attribution and compiler-isolation cycle establish these endpoints:

| Area | Status | Reopen condition |
|---|---|---|
| D64 M64xN128/eight-warps | Production endpoint | A source-distinct representation must remove measured work without the rejected mixed register/lane repack, spill, extra barrier, or global contribution change, then clear all resource, correctness, sanitizer, accuracy, and ABBA gates |
| D128 M64xN64/eight-warps | Maintained endpoint | A new representation must beat the retained matrix mirror and grouped dQ while preserving the double-buffer lifetime, `78,112`-byte residency, four barriers, and D64 machine identity |
| D64 M64xN256/sixteen-warps | Experimental endpoint for the current ownership model | A new accumulator count, storage, or workspace contract must first prove `<=128` registers and zero stack/local traffic in every raw/smooth tail/aligned image; source-local deletion, warp-id specialization, launch-bounds, and shared-spill variants are closed |
...
|---|---|
| Native preprocessing and dQ conversion cleanup | Revisit if matched phase timing exceeds about `10%` of end-to-end latency; the current D128 Flash win leaves this gap below that threshold |
